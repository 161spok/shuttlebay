...
# Sidebar content
st.sidebar.subheader("Info app")
st.sidebar.info("Questa app usa il modello 🤗HuggingFace's [google/pegasus-cnn_dailymail](https://huggingface.co/google/pegasus-cnn_dailymail).\
                 \nIl codice sorgente è su [quì](https://github.com/ivnlee/streamlit-text-summarizer).")
st.sidebar.write("\n\n")
st.sidebar.markdown("**Ottieni una free API key da HuggingFace:**")
st.sidebar.markdown("* Crea un [free account](https://huggingface.co/join) o [login](https://huggingface.co/login)")
st.sidebar.markdown("* Vai in **Settings** e poi **Access Tokens**")
st.sidebar.markdown("* Crea un nuono Token (seleziona il ruolo 'read')")
st.sidebar.markdown("* Incolla la API key nel text box")
st.sidebar.divider()
st.sidebar.write("Please make sure your article is in English and is not behind a paywall.")
st.sidebar.write("\n\n")
st.sidebar.divider()
st.sidebar.caption("Created by [Federico Pivoli]🎈.")
...
fetch_button = st.button("Carica articolo")
